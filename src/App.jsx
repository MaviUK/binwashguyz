...
const BOOKING_EMAIL = "aabincleaning@gmail.com"; // recipient via Resend
// ==================================================
  ...
  return (
    <section className="py-10 border-y border-[#103010] bg-[#003040]">
      <div className="max-w-6xl mx-auto px-4 flex items-center justify-between gap-6">
        <h3 className="text-2xl md:text-3xl font-extrabold text-white">Ready for fresh bins?</h3>
      </div>
    </section>
  );
}
...
function MobileActionBar({ onBook }) {
  const minimalMsg = encodeURIComponent(`Hi, I'd like to book a bin clean with ${BUSINESS_NAME}.`);
  ...
  async function handleEmail(e) {
    ...
    try {
      await fetch("/.netlify/functions/sendBookingEmail", {
        ...
        body: JSON.stringify({ business: BUSINESS_NAME, ...form }),
      });
...
function FieldShell({ label, children, className = "" }) {
  return <label className={`block ${className}`}><div className="text-xs mb-1 text-[#f0e0b0]/80">{label}</div>{children}</label>;
}
