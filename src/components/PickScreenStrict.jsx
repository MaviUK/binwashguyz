...
import { findCurrentGameweekNumber } from '../lib/findCurrentGameweek'
import { loadPlayerMatchupFromSupabase } from '../lib/loadPlayerMatchup'
import './PickScreen.css'

const PICK_SCREEN_VERSION = 'strict-next-week-v2'
...
export default function PickScreenStrict() {
  ...
  async function loadCurrentGameweek(options = {}) {
    ...
    try {
      const userId = await getUserId()
      const currentGameweekNumber = await findCurrentGameweekNumber(supabase, userId)
  ...
  async function submitPicks() {
    ...
    try {
      const userId = await getUserId()
      if (!gameweek?.id) throw new Error('Load your saved gameweek before submitting picks.')
      if (!matchup?.id) throw new Error('Load your matchup before submitting picks.')
      if (selectedIds.length !== 3) throw new Error('Select exactly 3 teams before submitting.')
      ...
      const response = await fetch('/.netlify/functions/saveEntryPicks', {
        ...
        body: JSON.stringify({ userId, gameweekId: gameweek.id, assignedSide, picks }),
      })
      const data = await response.json()
      if (!response.ok) throw new Error(data.message || data.error || 'Could not save picks.')
  ...
  return (
    <section className="pick-panel">
      ...
      <div className="builder-controls">
        <label>
          Gameweek
          <input type="number" min="1" max="38" value={gameweekNumber} onChange={(event) => setGameweekNumber(event.target.value)} />
        </label>
        <button type="button" onClick={() => loadCurrentGameweek()} disabled={loading}>{loading ? 'Loading...' : 'Load current week'}</button>
        <button type="button" className="secondary-button" onClick={() => loadGameweekAndMatchup()} disabled={loading}>Load selected week</button>
      </div>

      {!gameweek && !loading && <div className="empty-state">Load the current gameweek to see your VS and this week&apos;s 10 fixtures.</div>}

      {gameweek && matchup && (
        <div className="week-matchup-card">
          <p className="eyebrow small">Gameweek {gameweek.gameweek_number}</p>
          <div className="week-versus-row"><strong>{homeFantasyTeam}</strong><span>vs</span><strong>{awayFantasyTeam}</strong></div>
          <div className="week-meta-row"><span>You are {assignedSide.toUpperCase()}</span><span>Deadline: {gameweek.deadline_at ? formatKickoff(gameweek.deadline_at) : 'TBC'}</span><span>{fixtures.length}/10 fixtures</span></div>
        </div>
      )}

      {fixtures.length > 0 && (
        <div className="weekly-fixtures-block">
          <div className="panel-header compact-header"><div><p className="eyebrow small">This week&apos;s fixtures</p><h2>Pick 3 {assignedSide} teams</h2></div></div>
          <div className="week-fixture-list">
            {fixtures.map((fixture, index) => {
              ...
              return (
                <button key={fixture.id} type="button" className={`week-fixture-card ${selected ? 'selected' : ''}`} onClick={() => togglePick(fixture.id)}>
                  <span className="fixture-number">{index + 1}</span>
                  <span className="fixture-main"><strong>{fixture.home_team_name}</strong><em>vs</em><strong>{fixture.away_team_name}</strong></span>
                  <span className="fixture-pick-text">Your pick: <strong>{eligibleTeamName}</strong><small>Opponent side: {ineligibleTeamName}</small></span>
                </button>
              )
            })}
          </div>
      ...
      {fixtures.length > 0 && <button type="button" onClick={submitPicks} disabled={saving || loading || selectedIds.length !== 3}>{saving ? 'Submitting...' : 'Submit 3 picks'}</button>}
